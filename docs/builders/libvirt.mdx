...
modeline: |
  vim: set ft=pandoc:
description: |
  The Libvirt Packer builder is able to create KVM virtual machine images.
...
Type: `libvirt`
Artifact BuilderId: `transcend.libvirt`

The Libvirt Packer builder is able to create [KVM](http://www.linux-kvm.org) virtual
machine images.
...
down. The result of the Libvirt builder is a directory containing the image file
...
## Libvirt Specific Configuration Reference
...
@include 'builder/libvirt/CommConfig-not-required.mdx'
...
upstream bug with libvirt, and the best solution for now is to remove the
file/directory or run in another directory.

Some users have reported issues with incorrect keymaps using libvirt version 2.11.
This is a bug with libvirt, and the solution is to upgrade, or downgrade to 2.10.1
or earlier.

#### Corrupted image after Packer calls libvirt-img convert on OSX

Due to an upstream bug with `libvirt-img convert` on OSX, sometimes the libvirt-img
