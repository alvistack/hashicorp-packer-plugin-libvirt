...
//go:generate packer-sdc mapstructure-to-hcl2 -type Config,LibvirtImgArgs
...
type Config struct {
	...
	// Use iso from provided url. Libvirt must support
	...
	// [Libvirt 2.12.0](https://wiki.libvirt.org/ChangeLog/2.12#Host_support).
	// You may encounter issues unrelated to Packer when using these.  You may need to
	// add [ "-global", "virtio-pci.disable-modern=on" ] to `libvirtargs` depending on the
	// guest operating system.
	//
	// ~> For `whpx`, note that [Stefan Weil's Libvirt for Windows distribution](https://libvirt.weilnetz.de/w64/)
	// does not include WHPX support and users may need to compile or source a
	// build of Libvirt for Windows themselves with WHPX support.
	...
	// (exabyte, 1024P)  are supported. 'b' is ignored. Per libvirt-img documentation.
	...
	// The firmware file to be used by Libvirt, which is to be set by the -bios
	// option of Libvirt. Particularly, this option can be set to use EFI instead
	// of BIOS, by using "OVMF.fd" from OpenFirmware.
	// If unset, no -bios option is passed to Libvirt, using the default of Libvirt.
	// Also see the Libvirt documentation.
	...
	// device names. The Libvirt builder uses `virtio` by default.
	//
	...
	// running Libvirt on RHEL or a RHEL variant such as CentOS, you *must* choose
	...
	// libvirt-img resize.  Set this option to true to disable resizing.
	// Defaults to false.
	...
	// When the value is "off" we don't set the flag in the libvirt command, so that
	// Packer still works with old versions of Libvirt that don't have this option.
	DetectZeroes string `mapstructure:"disk_detect_zeroes" required:"false"`
	// Packer compacts the QCOW2 image using
	// libvirt-img convert.  Set this option to true to disable compacting.
	// Defaults to false.
	SkipCompaction bool `mapstructure:"skip_compaction" required:"false"`
	// Apply compression to the QCOW2 disk file
	// using libvirt-img convert. Defaults to false.
	...
	// `libvirt-img convert` on OSX, sometimes the libvirt-img convert call will
	...
	// https://bugs.launchpad.net/libvirt/+bug/1776920 for more details.
	Format string `mapstructure:"format" required:"false"`
	// Packer defaults to building Libvirt virtual machines by
	...
	// whether the ISO URL supplied is actually a bootable Libvirt image. When
	...
	// The type of machine emulation to use. Run your libvirt binary with the
	...
	// `vmxnet3`, `i82558a` or `i82558b`. The Libvirt builder uses `virtio-net` by
	// default.
	...
	// Allows complete control over the libvirt command line (though not libvirt-img).
	...
	//
	// ~> **Warning:** The libvirt command line allows extreme flexibility, so
	...
	// libvirt-system-x86 command. The arguments are all printed for review, and
	// you can use those arguments along with the template engines allowed
	// by libvirt-args to set up a working configuration that includes both the
	...
	// the builder and not otherwise conflicting with the libvirtargs):
	//
	// ```text
	// libvirt-system-x86 -m 1024m --no-acpi -netdev
	// user,id=mynet0,hostfwd=hostip:hostport-guestip:guestport -device
	// virtio-net,netdev=mynet0"
	// ```
	//
	// ~> **Windows Users:** [Libvirt for Windows](https://libvirt.weilnetz.de/)
	// builds are available though an environmental variable does need to be
	// set for Libvirt for Windows to redirect stdout to the console instead of
	// stdout.txt.
	//
	// The following shows the environment variable that needs to be set for
	// Windows Libvirt support:
	//
	// ```text
	...
	// `libvirtargs` to map to WinRM's default port of `5985` or whatever value
	...
	LibvirtArgs [][]string `mapstructure:"libvirtargs" required:"false"`
	// A map of custom arguments to pass to libvirt-img commands, where the key
	...
	// ```
	// Please note
	// that unlike libvirtargs, these commands are not split into switch-value
	// sub-arrays, because the basic elements in libvirt-img calls are  unlikely
	// to need an actual override.
	// The arguments will be constructed as follows:
	// - Convert:
	// 	Default is `libvirt-img convert -O $format $sourcepath $targetpath`. Adding
	// 	arguments ["-foo", "bar"] to libvirt_img_args.convert will change this to
	// 	`libvirt-img convert -foo bar -O $format $sourcepath $targetpath`
	// - Create:
	// 	Default is `create -f $format $targetpath $size`. Adding arguments
	// 	["-foo", "bar"] to libvirt_img_args.create will change this to
	// 	"create -f qcow2 -foo bar target.qcow2 1234M"
	// - Resize:
	// 	Default is `libvirt-img resize -f $format $sourcepath $size`. Adding
	// 	arguments ["-foo", "bar"] to libvirt_img_args.resize will change this to
	// 	`libvirt-img resize -f $format -foo bar $sourcepath $size`
	LibvirtImgArgs LibvirtImgArgs `mapstructure:"libvirt_img_args" required:"false"`
	// The name of the Libvirt binary to look for. This
	// defaults to libvirt-system-x86_64, but may need to be changed for
	// some platforms. For example libvirt-kvm, or libvirt-system-i386 may be a
	// better choice for some systems.
	LibvirtBinary string `mapstructure:"libvirt_binary" required:"false"`
	...
	// to libvirt, allowing it to choose the default. This may be needed when running
	// under macOS, and getting errors about sdl not being available.
	UseDefaultDisplay bool `mapstructure:"use_default_display" required:"false"`
	// What Libvirt -display option to use. Defaults to gtk, use none to not pass the
	// -display option allowing Libvirt to choose the default. This may be needed when
	...
	// The minimum port cannot be set below 5900 due to a quirk in how Libvirt parses
	// vnc display address.
	...
	// `virtio-scsi`. The Libvirt builder uses `virtio` by default.
...
func (c *Config) Prepare(raws ...interface{}) ([]string, error) {
	...
	if c.LibvirtBinary == "" {
		c.LibvirtBinary = "libvirt-system-x86_64"
	}
	...
	if c.LibvirtArgs == nil {
		c.LibvirtArgs = make([][]string, 0)
	}
