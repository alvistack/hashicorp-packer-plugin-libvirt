...
func (s *stepCopyDisk) Run(ctx context.Context, state multistep.StateBag) multistep.StepAction {
	...
	// This also serves as a workaround for a Libvirt bug: https://bugs.launchpad.net/libvirt/+bug/1776920
	ext := filepath.Ext(isoPath)
	if len(ext) >= 1 && ext[1:] == s.Format && len(s.LibvirtImgArgs.Convert) == 0 {
		ui.Message("File extension already matches desired output format. " +
			"Skipping libvirt-img convert step")
	...
	if err := driver.LibvirtImg(command...); err != nil {
...
func (s *stepCopyDisk) buildConvertCommand(sourcePath, targetPath string) []string {
	...
	command = append(command, s.LibvirtImgArgs.Convert...)
