...
func Test_StepCopySkip(t *testing.T) {
	...
	for _, tc := range testcases {
		...
		if d.CopyCalled || d.LibvirtImgCalled {
...
func Test_StepCopyCalled(t *testing.T) {
	...
	if d.LibvirtImgCalled {
		t.Fatalf("Should not have called libvirt-img when formats match")
	}
}

func Test_StepLibvirtImgCalled(t *testing.T) {
	step := stepCopyDisk{
		DiskImage: true,
		Format:    "raw",
		VMName:    "output.qcow2",
	...
	if !d.LibvirtImgCalled {
		t.Fatalf("Should have called libvirt-img since extensions don't match")
	}
}

func Test_StepLibvirtImgCalledWithExtraArgs(t *testing.T) {
	step := &stepCopyDisk{
		DiskImage: true,
		Format:    "raw",
		VMName:    "output.qcow2",
		LibvirtImgArgs: LibvirtImgArgs{
