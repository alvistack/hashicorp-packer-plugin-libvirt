...
func (s *stepRun) Run(ctx context.Context, state multistep.StateBag) multistep.StepAction {
	...
	// Figure out version of libvirt; store on step for later use
	rawVersion, err := driver.Version()
	if err != nil {
		err := fmt.Errorf("Error determining libvirt version: %s", err)
		s.ui.Error(err.Error())
		return multistep.ActionHalt
	}
	libvirtVersion, err := version.NewVersion(rawVersion)
	if err != nil {
		err := fmt.Errorf("Error parsing libvirt version: %s", err)
		s.ui.Error(err.Error())
		return multistep.ActionHalt
	}
	v2 := version.Must(version.NewVersion("2.0"))

	s.atLeastVersion2 = libvirtVersion.GreaterThanOrEqual(v2)

	// Generate the libvirt command
	command, err := s.getCommandArgs(config, state)
	if err != nil {
		err := fmt.Errorf("Error processing LibvirtArgs: %s", err)
		s.ui.Error(err.Error())
		return multistep.ActionHalt
	}

	// run the libvirt command
	if err := driver.Libvirt(command...); err != nil {
...
func (s *stepRun) getDefaultArgs(config *Config, state multistep.StateBag) map[string]interface{} {
	...
	if floppyPathRaw, ok := state.GetOk("floppy_path"); ok {
		...
		log.Println("Libvirt Builder has no floppy files, not attaching a floppy.")
	}

	// Configure GUI display
	if !config.Headless {
		if s.atLeastVersion2 {
			// FIXME: "none" is a valid display option in libvirt but we have
			// departed from the libvirt usage here to instaed mean "let libvirt
			// set a reasonable default". We need to deprecate this behavior
			// and let users just set "UseDefaultDisplay" if they want to let
			// libvirt do its thing.
			...
			s.ui.Message("WARNING: The version of libvirt  on your host doesn't support display mode.\n" +
...
func (s *stepRun) getDeviceAndDriveArgs(config *Config, state multistep.StateBag) ([]string, []string) {
	...
	if s.atLeastVersion2 {
		...
		if v, ok := state.GetOk("libvirt_disk_paths"); ok {
...
func (s *stepRun) applyUserOverrides(defaultArgs map[string]interface{}, config *Config, state multistep.StateBag) ([]string, error) {
	...
	if len(config.LibvirtArgs) > 0 {
		s.ui.Say("Overriding default Libvirt arguments with libvirtargs template option...")
		...
		type libvirtArgsTemplateData struct {
			HTTPIP      string
			HTTPPort    int
			HTTPDir     string
		...
		ictx := config.ctx
		ictx.Data = libvirtArgsTemplateData{
			HTTPIP:      httpIp,
			HTTPPort:    httpPort,
		...
		// Interpolate each string in libvirtargs
		newLibvirtArgs, err := processArgs(config.LibvirtArgs, &ictx)
		if err != nil {
			return nil, err
		}

		// Libvirt supports multiple appearances of the same switch. This means
		// each key in the args hash will have an array of string values
		for _, libvirtArgs := range newLibvirtArgs {
			key := libvirtArgs[0]
			val := strings.Join(libvirtArgs[1:], "")
