...
func runTestState(t *testing.T, config *Config) multistep.StateBag {
	...
	state.Put("libvirt_disk_paths", []string{})
...
func Test_UserOverrides(t *testing.T) {
	...
	testcases := []testCase{
		...
		},
		{
			&Config{
				VMName:   "myvm",
				LibvirtArgs: [][]string{{"-display", "partydisplay"}},
			},
			[]string{
		...
		},
		{
			&Config{
				...
				LibvirtArgs:  [][]string{{"-device", "somerandomdevice"}},
			},
			[]string{
				"-display", "gtk",
	...
	testcases := []testCase{
		...
		},
		{
			...
			map[string]interface{}{
				...
				"libvirt_disk_paths": []string{"path_to_output"},
			},
			&stepRun{
				DiskImage:       true,
				atLeastVersion2: true,
		...
		},
		{
			...
			map[string]interface{}{
				...
				"libvirt_disk_paths": []string{"libvirtpath1", "libvirtpath2"},
			},
			&stepRun{
				atLeastVersion2: true,
				ui:              packersdk.TestUi(t),
			...
			[]string{
				...
				"-drive", "if=none,file=libvirtpath1,id=drive0,cache=writeback,discard=,format=qcow2",
				"-drive", "if=none,file=libvirtpath2,id=drive1,cache=writeback,discard=,format=qcow2",
				"-drive", "file=/path/to/test.iso,media=cdrom",
				"-drive", "file=fake_cd_path.iso,media=cdrom",
			},
		...
		},
		{
			...
			[]string{
				...
				"-device", "scsi-hd,bus=scsi0.0,drive=drive0",
				"-device", "scsi-hd,bus=scsi0.0,drive=drive1",
				"-drive", "if=none,file=libvirtpath1,id=drive0,cache=writeback,discard=,format=qcow2,detect-zeroes=on",
				"-drive", "if=none,file=libvirtpath2,id=drive1,cache=writeback,discard=,format=qcow2,detect-zeroes=on",
				"-drive", "file=/path/to/test.iso,media=cdrom",
				"-drive", "file=fake_cd_path.iso,media=cdrom",
			},
		...
		},
		{
			...
			map[string]interface{}{
				// when disk image is false, we will always have at least one
				// disk path: the one we create to be the main disk.
				"libvirt_disk_paths": []string{"output/dir/path/mydisk.qcow2"},
			},
			&stepRun{
				atLeastVersion2: true,
				ui:              packersdk.TestUi(t),
		...
		},
		{
			...
			[]string{
				"-display", "gtk",
				"-boot", "once=d",
				"-drive", "file=libvirtpath1,if=virtio,cache=writeback,discard=,format=qcow2,detect-zeroes=",
				"-drive", "file=libvirtpath2,if=virtio,cache=writeback,discard=,format=qcow2,detect-zeroes=",
				"-drive", "file=fake_cd_path.iso,media=cdrom",
				"-drive", "file=/path/to/test.iso,media=cdrom",
			},
