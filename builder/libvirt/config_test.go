...
func TestBuilderPrepare_LibvirtArgs(t *testing.T) {
	var c Config
	config := testConfig()

	// Test with empty
	delete(config, "libvirtargs")
	warns, err := c.Prepare(config)
	if len(warns) > 0 {
		t.Fatalf("bad: %#v", warns)
	}
	...
	if !reflect.DeepEqual(c.LibvirtArgs, [][]string{}) {
		t.Fatalf("bad: %#v", c.LibvirtArgs)
	}

	// Test with a good one
	config["libvirtargs"] = [][]interface{}{
		{"foo", "bar", "baz"},
	}
	...
	if !reflect.DeepEqual(c.LibvirtArgs, expected) {
		t.Fatalf("bad: %#v", c.LibvirtArgs)
	}
}
...
func TestBuilderPrepare_LoadLibvirtImgArgs(t *testing.T) {
	var c Config
	config := testConfig()
	config["libvirt_img_args"] = map[string][]string{
	...
	assert.Equal(t, []string{"-o", "preallocation=full"},
		c.LibvirtImgArgs.Convert, "Convert args not loaded properly")
	assert.Equal(t, []string{"-foo", "bar"},
		c.LibvirtImgArgs.Resize, "Resize args not loaded properly")
	assert.Equal(t, []string{"-baz", "bang"},
		c.LibvirtImgArgs.Create, "Create args not loaded properly")
}
