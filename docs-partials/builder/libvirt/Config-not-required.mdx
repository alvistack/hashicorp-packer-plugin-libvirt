...
- `iso_skip_cache` (bool) - Use iso from provided url. Libvirt must support
...
- `firmware` (string) - The firmware file to be used by Libvirt, which is to be set by the -bios
...
- `headless` (bool) - Packer defaults to building Libvirt virtual machines by
...
- `machine_type` (string) - The type of machine emulation to use. Run your libvirt binary with the
...
- `libvirtargs` ([][]string) - Allows complete control over the libvirt command line (though not libvirt-img).
...
- `libvirt_img_args` (LibvirtImgArgs) - A map of custom arguments to pass to libvirt-img commands, where the key
...
- `libvirt_binary` (string) - The name of the Libvirt binary to look for. This
...
- `display` (string) - What Libvirt -display option to use. Defaults to gtk, use none to not pass the
